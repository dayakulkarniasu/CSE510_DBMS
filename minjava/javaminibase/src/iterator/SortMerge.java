...
public class SortMerge extends Iterator implements GlobalConst {
	...
	public SortMerge(AttrType in1[], int len_in1, short s1_sizes[], AttrType in2[], int len_in2, short s2_sizes[],
			...
			boolean in1_sorted, boolean in2_sorted, MapOrder order,

			CondExpr outFilter[], FldSpec proj_list[], int n_out_flds)
			throws JoinNewFailed, JoinLowMemory, SortException, MapUtilsException, IOException

	{
		...
		nOutFlds = n_out_flds;
		try {
			ts_size = MapUtils.setup_op_map(Jmap, Jtypes, in1, len_in1, in2, len_in2, s1_sizes, s2_sizes, proj_list,
					n_out_flds);
		} catch (Exception e) {
	...
	public Map get_next() throws IOException, JoinsException, IndexException, InvalidTupleSizeException,
		...
		while (true) {
			if (process_next_block) {
				...
				comp_res = MapUtils.CompareMapWithMap(sortFldType, map1, jc_in1, map2, jc_in2);
				...
				while (MapUtils.CompareMapWithMap(sortFldType, map1, jc_in1, TempMap1, jc_in1) == 0) {
				...
				while (MapUtils.CompareMapWithMap(sortFldType, map2, jc_in2, TempMap2, jc_in2) == 0) {
			...
			if (PredEval.Eval(OutputFilter, TempMap1, TempMap2, _in1, _in2) == true) {
				Projection.Join(TempMap1, _in1, TempMap2, _in2, Jmap, perm_mat, nOutFlds);
				return Jmap;
			}
		}
	}
	...
	public void close() throws JoinsException, IOException, IndexException {
		if (!closeFlag) {
			...
			try {
				...
				// io_buf1.close();// shunchi
				// io_buf2.close();// shunchi
