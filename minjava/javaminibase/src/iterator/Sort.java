...
public class Sort extends Iterator implements GlobalConst {
  ...
  private int generate_runs(int max_elems, AttrType sortFldType, int sortFldLen)
      throws IOException, SortException, UnknowAttrType, MapUtilsException, JoinsException, Exception {
    Map map;
    pnode cur_node;
    ...
    while (true) {
      ...
      comp_res = MapUtils.CompareMapWithValue(sortFldType, cur_node.map, _sort_fld, lastElem); // need
                                                                                               // tuple_utils.java
   ...
   * @exception SortException something went wrong in the lower layer.
   */
  public Sort(AttrType[] in, short len_in, short[] str_sizes, Iterator am, int sort_fld, MapOrder sort_order,
   ...
   * @exception SortException something went wrong in the lower layer.
   */
  public Sort(AttrType[] in, short len_in, short[] str_sizes, Iterator am, int[] sort_flds, MapOrder sort_order,
      int[] sort_fld_lens, int n_pages) throws IOException, SortException {
    single = false;
    _in = new AttrType[len_in];
    n_cols = len_in;
    int n_strs = 0;
   ...
   * @exception Exception       other exceptions
   */
  public Map get_next() throws IOException, SortException, UnknowAttrType, LowMemException, JoinsException, Exception {
    if (first_time) {
   ...
   * @exception JoinsException from <code>Iterator.get_next()</code>
   */
  private int generate_runs(int max_elems, AttrType[] sortFldTypes, int[] sortFldLens)
      throws IOException, SortException, UnknowAttrType, MapUtilsException, JoinsException, Exception {
    Map map;
    pnode cur_node;
    pnodeSplayPQ Q1 = new pnodeSplayPQ(_sort_flds, sortFldTypes, order);
    pnodeSplayPQ Q2 = new pnodeSplayPQ(_sort_flds, sortFldTypes, order);
    pnodeSplayPQ pcurr_Q = Q1;
    pnodeSplayPQ pother_Q = Q2;
   ...
   * @exception UnknowAttrType attrSymbol or attrNull encountered
   */
  private void MIN_VAL(Map lastElem, AttrType[] sortFldTypes)
      throws IOException, FieldNumberOutOfBoundException, UnknowAttrType {
   ...
   * @exception UnknowAttrType attrSymbol or attrNull encountered
   */
  private void MAX_VAL(Map lastElem, AttrType[] sortFldTypes)
      throws IOException, FieldNumberOutOfBoundException, UnknowAttrType {
