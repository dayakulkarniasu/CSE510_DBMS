...
public class RowJoin extends Iterator {
   ...
   */
  // public RowJoin(AttrType in1[], int len_in1, short t1_str_sizes[], AttrType
  // in2[], int len_in2,
  // short t2_str_sizes[], int amt_of_mem, Iterator am1, String relationName,
  // CondExpr outFilter[],
  // CondExpr rightFilter[], FldSpec proj_list[], int n_out_flds) throws
  // IOException, NestedLoopException {
  public RowJoin(int amtofmem, Stream LeftBigT, String RightBigTName, String ColumnName) throws IOException,
      NestedLoopException, iterator.FileScanException, iterator.MapUtilsException, iterator.InvalidRelation {
    ...
    try {
      t_size = MapUtils.setup_op_map(Jmap, Jtypes, Schema, MapFldCount, Schema, MapFldCount, MapStrLengths,
          MapStrLengths, OutputSchema, nOutFlds);
    ...
    try {
      ...
      FileScan fscan = new FileScan(RightBigTName, Schema, MapStrLengths, (short) MapFldCount, MapFldCount,
          OutputSchema, RightFilter);
   ...
   *
   */
  // public Map get_next2() throws IOException, JoinsException, IndexException,
  // InvalidTupleSizeException,
  // InvalidTypeException, PageNotReadException, MapUtilsException,
  // PredEvalException, SortException,
  ...
  //// peek.setHdr((short) MapSchema.MapFldCount(),MapSchema.MapAttrType(),
  // MapSchema.MapStrLengths());
  //// if(MapUtils.CompareMapWithMap(new AttrType(AttrType.attrString), outer_map,
  // GlobalConst.ROW_LABEL, peek, GlobalConst.ROW_LABEL) == 0 &&
  //// MapUtils.CompareMapWithMap(new AttrType(AttrType.attrString), outer_map,
  // GlobalConst.COL_LABEL, peek, GlobalConst.COL_LABEL) == 0){
  //// //if(outer_map.getRowLabel().equalsIgnoreCase(peek.getRowLabel())){
  //// try{
  //// outer_map = peek;//new Map(peek.getMapByteArray(), 0,
  // GlobalConst.MAP_LEN);;//new Map(peek.getMapByteArray(), 0 ,
  // GlobalConst.MAP_LEN);
  //// //peek.setHdr((short) MapSchema.MapFldCount(),MapSchema.MapAttrType(),
  // MapSchema.MapStrLengths());
  //// }
  ...
  // Projection.RowJoin(outer_map, _in1, inner_map, _in2, Jmap, perm_mat,
  // nOutFlds);
  // //inner = null;
  // //inner.closescan();
  ...
  public Map get_next() throws IOException, JoinsException, IndexException, InvalidTupleSizeException,
    ...
    do {
      ...
      if (get_from_outer == true) {
        ...
        if (inner != null) // If this not the first time,
        {
          System.out.println("INNER NOT NULL");
          // had an inner close
          inner.closescan();
          inner = null;
        }
        ...
        if (outer_map == null) {
          System.out.println("OUTER MAP NULL");
          done = true;
          if (inner != null) {
            System.out.println("THE INNER-INNER NOT NULL");
            inner.closescan();
            inner = null;
          }
          return null;
      ...
      if (outer_map != null) {
        ...
        MID mid = new MID();
        inner_map = inner.getNext(mid);
        while (inner_map != null) {
          boolean latestFound = false;
          Map inner_peek = null;
          while (!latestFound) {
            inner_map.setHdr((short) in2_len, _in2, t2_str_sizescopy);
            inner_peek = inner.getNext(mid);
            if (inner_peek != null) {
              // If RL CL match, the current map is not the latest value TS
              inner_peek = new Map(inner_peek.getMapByteArray(), 0, GlobalConst.MAP_LEN);
              inner_peek.setHdr((short) MapSchema.MapFldCount(), MapSchema.MapAttrType(), MapSchema.MapStrLengths());
              if (MapUtils.CompareMapWithMap(new AttrType(AttrType.attrString), inner_map, GlobalConst.ROW_LABEL,
                  inner_peek, GlobalConst.ROW_LABEL) == 0
                  && MapUtils.CompareMapWithMap(new AttrType(AttrType.attrString), inner_map, GlobalConst.COL_LABEL,
                      inner_peek, GlobalConst.COL_LABEL) == 0) {
                inner_map = inner_peek;
              } else {
                latestFound = true;
              }
            } else {
  ...
  private void findLatestMap(Iterator am, boolean eof) throws Exception {
    // boolean eof = false;
    // Map peek = null;
    while (outer_map != null) {
      peek = am.get_next();
      if (peek != null) {
        ...
        if (MapUtils.CompareMapWithMap(new AttrType(AttrType.attrString), outer_map, GlobalConst.ROW_LABEL, peek,
            GlobalConst.ROW_LABEL) == 0
            && MapUtils.CompareMapWithMap(new AttrType(AttrType.attrString), outer_map, GlobalConst.COL_LABEL, peek,
                GlobalConst.COL_LABEL) == 0) {
          try {
            outer_map = peek;
          } catch (Exception e) {
            e.printStackTrace();
