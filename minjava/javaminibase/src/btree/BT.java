...
public class BT implements GlobalConst {
  ...
  public final static int keyCompare(KeyClass key1, KeyClass key2) throws KeyNotMatchException {
    ...
    } else if ((key1 instanceof CombinedKey) && (key2 instanceof CombinedKey)) {
      String[] skey1 = ((CombinedKey) key1).getKey();
      String[] skey2 = ((CombinedKey) key2).getKey();
      if(skey1[0].compareTo(skey2[0]) == 0){
        return skey1[0].compareTo(skey2[0]);
      } else{
        return skey1[1].compareTo(skey2[1]);
      }
    }
  ...
  protected final static KeyDataEntry getEntryFromBytes(byte[] from, int offset, int length, int keyType,
    ...
    try {
      ...
      } else if (keyType == AttrType.attrCombined) {
        key = new CombinedKey(Convert.getStrValue(offset, from, STR_LEN),
                Convert.getStrValue(offset + STR_LEN, from, STR_LEN));
      }
      else
        throw new KeyNotMatchException(null, "key types do not match");
  ...
  protected final static byte[] getBytesFromEntry(KeyDataEntry entry)
    ...
    try {
      ...
      }else if (entry.key instanceof CombinedKey) {
        String[] keys = ((CombinedKey) entry.key).getKey();
        Convert.setStrValue(keys[0], 0, data);
        Convert.setStrValue(keys[1], STR_LEN, data);
      }
      else
        throw new KeyNotMatchException(null, "key types do not match");
  ...
  public static void printPage(PageId pageno, int keyType)
      throws IOException, IteratorException, ConstructPageException, HashEntryNotFoundException, ReplacerException,
    ...
    if (sortedPage.getType() == NodeType.INDEX) {
      ...
      for (KeyDataEntry entry = indexPage.getFirst(mid); entry != null; entry = indexPage.getNext(mid)) {
        if (keyType == AttrType.attrInteger)
          System.out
              .println(i + " (key, pageId):   (" + (IntegerKey) entry.key + ",  " + (IndexData) entry.data + " )");
        if (keyType == AttrType.attrString)
          System.out.println(i + " (key, pageId):   (" + (StringKey) entry.key + ",  " + (IndexData) entry.data + " )");
      ...
      for (KeyDataEntry entry = leafPage.getFirst(mid); entry != null; entry = leafPage.getNext(mid)) {
        if (keyType == AttrType.attrInteger)
          System.out.println(
              i + " (key, [pageNo, slotNo]):   (" + (IntegerKey) entry.key + ",  " + (LeafData) entry.data + " )");
        if (keyType == AttrType.attrString)
          System.out
              .println(i + " (key, [pageNo, slotNo]):   (" + (StringKey) entry.key + ",  " + (LeafData) entry.data);
        if (keyType == AttrType.attrCombined)
          System.out.println(
                  i + " (key, [pageNo, slotNo]):   (" + (CombinedKey) entry.key + ",  " + (LeafData) entry.data + " )");
        i++;
      }
