...
public class BTIndexPage extends BTSortedPage {
   ...
   */
  public MID insertKey(KeyClass key, PageId pageNo) throws IndexInsertRecException {
    MID mid;
    KeyDataEntry entry;
    try {
   ...
   */
  MID deleteKey(KeyClass key) throws IndexFullDeleteException {
    KeyDataEntry entry;
    MID mid = new MID();
    ...
    try {
      ...
      if (direction == -1) { // 'this' is the left sibling of indexPage
        ...
        if ((getSlotLength(0) + available_space()) > ((MAX_SPACE - DPFIXED) / 2)) {
          ...
          firstEntry = BT.getEntryFromBytes(getpage(), getSlotOffset(0), getSlotLength(0), keyType, NodeType.INDEX);
