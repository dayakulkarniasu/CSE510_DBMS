...
public class BTreeFile extends IndexFile implements GlobalConst {
	...
	private void _destroyFile(PageId pageno) throws IOException, IteratorException, PinPageException,
		...
		if (sortedPage.getType() == NodeType.INDEX) {
			...
			PageId childId;
			KeyDataEntry entry;
			for (entry = indexPage.getFirst(mid); entry != null; entry = indexPage.getNext(mid)) {
	 ...
	 */
	public void insert(KeyClass key, MID mid) throws KeyTooLongException, KeyNotMatchException, LeafInsertRecException,
...

	{
		...
		if (trace != null) {
			trace.writeBytes("INSERT " + mid.pageNo + " " + mid.slotNo + " " + key + lineSep);
	...
	private KeyDataEntry _insert(KeyClass key, MID mid, PageId currentPageId)
...

	{
		...
		if (currentPage.getType() == NodeType.INDEX) {
			...
			for (tmpEntry = currentIndexPage.getFirst(delMid); tmpEntry != null; tmpEntry = currentIndexPage
					.getFirst(delMid)) {
			...
			for (tmpEntry = newIndexPage.getFirst(firstMid); (currentIndexPage.available_space() > newIndexPage
					.available_space()); tmpEntry = newIndexPage.getFirst(firstMid)) {
			...
			if (currentIndexPage.available_space() < newIndexPage.available_space()) {
				...
				currentIndexPage.deleteSortedRecord(
						new MID(currentIndexPage.getCurPage(), (int) currentIndexPage.getSlotCnt() - 1));
			}
			...
			if (BT.keyCompare(upEntry.key, tmpEntry.key) >= 0) {
				...
				currentIndexPage.deleteSortedRecord(new MID(currentIndexPage.getCurPage(), i));
			...
			for (tmpEntry = currentLeafPage.getFirst(firstMid); tmpEntry != null; tmpEntry = currentLeafPage
					.getFirst(firstMid)) {
			...
			KeyDataEntry undoEntry = null;
			for (tmpEntry = newLeafPage.getFirst(firstMid); newLeafPage.available_space() < currentLeafPage
					.available_space(); tmpEntry = newLeafPage.getFirst(firstMid)) {
				undoEntry = tmpEntry;
			...
			if (BT.keyCompare(key, undoEntry.key) < 0) {
				...
				if (currentLeafPage.available_space() < newLeafPage.available_space()) {
					...
					currentLeafPage.deleteSortedRecord(
							new MID(currentLeafPage.getCurPage(), (int) currentLeafPage.getSlotCnt() - 1));
				}
			}
	...
	/**
	 * delete leaf entry given its <key, rid> pair. `mid' is IN the data entry; it
	...
	BTLeafPage findRunStart(KeyClass lo_key, MID startmid) throws IOException, IteratorException, KeyNotMatchException,
	...
	private boolean NaiveDelete(KeyClass key, MID mid)
		...
		if (trace != null) {
			trace.writeBytes("DELETE " + mid.pageNo + " " + mid.slotNo + " " + key + lineSep);
			trace.writeBytes("DO" + lineSep);
			trace.writeBytes("SEARCH" + lineSep);
			trace.flush();
		}
	...
	private KeyClass _Delete(KeyClass key, MID mid, PageId currentPageId, PageId parentPageId)
			throws IndexInsertRecException, RedistributeException, IndexSearchException, RecordNotFoundException,
			DeleteRecException, InsertRecException, LeafRedistributeException, IndexFullDeleteException,
				...
				if (leafPage.delEntry(new KeyDataEntry(key, mid))) {
					...
					if ((4 + leafPage.available_space()) <= ((MAX_SPACE - HFPage.DPFIXED) / 2)) {
						...
						if (siblingPage.redistribute(leafPage, parentPage, direction, deletedKey)) {
							...
							for (tmpEntry = rightChild.getFirst(firstMid); tmpEntry != null; tmpEntry = rightChild
									.getFirst(firstMid)) {
		...
		if (sortPage.getType() == NodeType.INDEX) {
			...
			if ((4 /* sizeof slot */ + indexPage.available_space()) <= ((MAX_SPACE - HFPage.DPFIXED) / 2)) {
				...
				int pushKeySize = 0;
				if (direction == 1) {
					pushKeySize = BT.getKeyLength(parentPage.findKey(siblingPage.getFirst(new MID()).key));
				} else if (direction == -1) {
					pushKeySize = BT.getKeyLength(parentPage.findKey(indexPage.getFirst(new MID()).key));
				}
				...
				if (siblingPage.redistribute(indexPage, parentPage, direction, deletedKey)) {
					...
					// move all entries from rightChild to leftChild
					for (KeyDataEntry tmpEntry = rightChild.getFirst(firstMid); tmpEntry != null; tmpEntry = rightChild
							.getFirst(firstMid)) {
	...
	void trace_children(PageId id)
		...
		if (trace != null) {
			...
			if (sortedPage.getType() == NodeType.INDEX) {
				...
				for (entry = indexPage.getFirst(metaMid); entry != null; entry = indexPage.getNext(metaMid)) {
				...
				for (entry = leafPage.getFirst(metaMid); entry != null; entry = leafPage.getNext(metaMid)) {
