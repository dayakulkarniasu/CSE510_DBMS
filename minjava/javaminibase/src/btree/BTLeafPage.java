...
public class BTLeafPage extends BTSortedPage {
   ...
   */
  public MID insertRecord(KeyClass key, MID dataMid) throws LeafInsertRecException {
    KeyDataEntry entry;
   ...
   * 
   * @param mid It will be modified and the first mid in the leaf page will be
   ...
   */
  public KeyDataEntry getFirst(MID mid) throws IteratorException {
  ...
  public KeyDataEntry getNext(MID mid) throws IteratorException {
    KeyDataEntry entry;
    int i;
    try {
   ...
   * @exception IteratorException iterator error
   */
  public KeyDataEntry getCurrent(MID mid) throws IteratorException {
    mid.slotNo--;
    return getNext(mid);
  }
    ...
    try {
      if (direction == -1) { // 'this' is the left sibling of leafPage
        ...
        if ((getSlotLength(0) + available_space() + 8) > ((MAX_SPACE - DPFIXED) / 2)) {
          ...
          KeyDataEntry firstEntry;
          firstEntry = BT.getEntryFromBytes(getpage(), getSlotOffset(0), getSlotLength(0), keyType, NodeType.LEAF);
